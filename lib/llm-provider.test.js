...
describe('BedrockProvider', () => {
  ...
  describe('handleBedrockError', () => {
    it('should handle UnrecognizedClientException', () => {
      const error = new Error('The security token included in the request is invalid.');
      error.name = 'UnrecognizedClientException';
      
      const handled = provider.handleBedrockError(error);
      expect(handled.message).toContain('Invalid AWS credentials');
      expect(handled.message).toContain('Access Key and Secret Key');
    });
    ...
    it('should handle AccessDeniedException', () => {
      ...
      const handled = provider.handleBedrockError(error);
      expect(handled.message).toContain('Bedrock permissions');
      expect(handled.message).toContain('bedrock:InvokeModel');
    });
    ...
    it('should handle ResourceNotFoundException', () => {
      ...
      const handled = provider.handleBedrockError(error);
      expect(handled.message).toContain('not available in selected region');
      expect(handled.message).toContain('us-east-1');
    });
    ...
    it('should handle NetworkingError', () => {
      const error = new Error('Network failure');
      error.name = 'NetworkingError';
      
      const handled = provider.handleBedrockError(error);
      expect(handled.message).toContain('Network error');
      expect(handled.message).toContain('internet connection');
    });
    ...
    it('should handle TimeoutError', () => {
      ...
      const handled = provider.handleBedrockError(error);
      expect(handled.message).toContain('timed out');
    });
    ...
    it('should handle generic errors', () => {
      ...
      const handled = provider.handleBedrockError(error);
      expect(handled.message).toContain('Bedrock API error');
      expect(handled.message).toContain('Some other error');
    });
  });
});
